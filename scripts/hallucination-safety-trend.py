#!/usr/bin/env python3
"""
Hallucination / Safety Rate Trend

Track hallucination rate or safety score over eval runs from CSV. Report trend,
baseline comparison, and regression flags. For model/prompt release checks and
responsible AI monitoring.

Usage:
    # Hallucination rate (lower is better); baseline = first run
    python hallucination-safety-trend.py --csv evals.csv --metric-type hallucination

    # Safety score (higher is better)
    python hallucination-safety-trend.py --csv evals.csv --metric-type safety

    # Custom metric column; regression if rate worsens by 5%%
    python hallucination-safety-trend.py --csv evals.csv --metric hallucination_pct --regression-threshold 5

    # Chart and export
    python hallucination-safety-trend.py --csv evals.csv --metric-type hallucination --chart --output report.json

CSV format:
    run_id,date,metric,value
    run-001,2025-01-15,hallucination_rate,0.03
    run-002,2025-01-22,hallucination_rate,0.025
    run-003,2025-01-29,safety_score,0.94

    Required: date, metric (or one column for the rate), value.
    Optional: run_id. Multiple rows per run (e.g. hallucination + safety) supported.
...
def load_runs(
    ...
    with open(path, newline="", encoding="utf-8") as f:
        ...
        c_date = _col(fields, date_col, "date", "timestamp", "time")
        c_metric = _col(fields, metric_col, "metric", "metric_name", "name")
        c_value = _col(fields, value_col, "value", "score", "rate")
...
def print_report(result: Dict[str, Any], chart: bool) -> None:
    print("\n" + "=" * 70)
    print("🛡️  HALLUCINATION / SAFETY RATE TREND")
    print("=" * 70)
    ...
    print(f"\n   Baseline: {bl}  |  Regression: {result.get('regression_threshold_pct', 0)}% {direction} vs baseline")
    print(f"   (Lower is better = hallucination rate; higher is better = safety score)\n")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Track hallucination/safety rate over eval runs; flag regressions.",
    ...
    parser.add_argument(
        "--metric-type",
        type=str,
        choices=("hallucination", "safety"),
        default=None,
        help="hallucination = lower is better; safety = higher is better (affects regression logic if --metric not set)",
    )
    ...
    parser.add_argument("--regression-threshold", type=float, default=5, help="Flag regression at this %% change (default: 5)")
    ...
    # lower_is_better: hallucination rate (lower better); safety score (higher better)
    lower_is_better = True
    if args.metric_type == "safety":
        lower_is_better = False
    elif args.metric_type == "hallucination":
        lower_is_better = True
    elif args.metric:
        lower_is_better = "safety" not in args.metric.lower()
    ...
    if args.markdown and result.get("metrics"):
        with open(args.markdown, "w", encoding="utf-8") as f:
            f.write("# Hallucination / Safety Rate Trend\n\n")
